...
import { formatCurrency, formatPercent, formatMonths, formatYears } from '@/lib/utils';
...
export function ResultsPanel({ result, inputs, onExportPDF, isExporting }: ResultsPanelProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-obsidian-700/60 bg-obsidian-900/30 p-6 backdrop-blur-sm">
        ...
        <div className="mt-5 rounded-lg overflow-hidden h-2.5 bg-obsidian-800 flex">
          ...
          />
          <div
            className="bg-gold-400 transition-all duration-700 flex-1"
          />
        </div>
        <div className="flex gap-4 mt-2 text-xs text-obsidian-500">
          <span className="flex items-center gap-1.5"><span className="inline-block w-2 h-2 rounded-full bg-red-500/70" />Комиссия</span>
          <span className="flex items-center gap-1.5"><span className="inline-block w-2 h-2 rounded-full bg-amber-500/70" />Расходы</span>
          <span className="flex items-center gap-1.5"><span className="inline-block w-2 h-2 rounded-full bg-gold-400" />Прибыль</span>
        </div>
      </div>
...
function BreakdownRow({
  ...
  const colors = {
    ...
    highlight: { label: 'text-obsidian-100 font-semibold', value: 'text-gold-400 font-bold text-lg' },
  };

  return (
    <div className={`flex items-baseline justify-between py-2.5 ${type === 'highlight' ? '' : 'border-b border-obsidian-800/60'}`}>
      <span className={`text-sm ${colors[type].label}`}>{label}</span>
      <span className={`text-sm ${colors[type].value} tabular-nums`}>{value}</span>
    </div>
  );
}
